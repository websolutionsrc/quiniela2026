...
const API_R32_TREE_ORDER = [
  'fd-537417', 'fd-537418', 'fd-537415', 'fd-537416',
...
export function mvpLiveScorers() {
  const data = getData();
  const list = Array.isArray(data.mvpLiveScorers) && data.mvpLiveScorers.length
    ? data.mvpLiveScorers
    : mvpCandidates();
  return list.slice(0, 20).map(localizePlayerTeam);
}
export function actualMvp() {
  const data = getData();
  return data.mvpActual || inferActualMvpFromData(data) || CONFIG.mvp.actual || null;
}
...
function localizeData(data) {
  ...
  if (Array.isArray(data.mvpLiveScorers)) {
    out.mvpLiveScorers = data.mvpLiveScorers.map(localizePlayerTeam);
  }
  return out;
}
...
function finalFinishedIn(matches) {
  const fm = matches.find(m => m.phase === 'FINAL');
  return isFinished(fm);
}
function topScorerId(scorers) {
  const ranked = (Array.isArray(scorers) ? scorers : [])
    .filter(p => p?.id && Number.isFinite(Number(p.goals)))
    .map((p, i) => ({ ...p, goals: Number(p.goals), i }));
  if (!ranked.length) return null;
  ranked.sort((a, b) => b.goals - a.goals || a.i - b.i);
  return ranked[0].id;
}
function inferActualMvpFromData(data) {
  if (!data || !finalFinishedIn(data.matches || [])) return null;
  return topScorerId(data.mvpLiveScorers);
}
...
// ¿Está abierta la prediccion de la final?
...
export async function fetchFromAPI() {
  ...
  let liveScorers = [];
  if (groupPhaseFinishedIn(stableMatches)) {
    try {
      liveScorers = await fetchMvpCandidatesFromAPI(token);
    } catch (e) {
      if (!(previous?.mvpCandidatesLocked && Array.isArray(previous.mvpCandidates) && previous.mvpCandidates.length)) throw e;
      liveScorers = Array.isArray(previous.mvpLiveScorers) ? previous.mvpLiveScorers : [];
    }
    if (liveScorers.length) {
      payload.mvpLiveScorers = liveScorers;
      payload.mvpLiveScorersAt = payload.fetchedAt;
      payload.mvpLiveScorersSource = 'api-scorers';
    }
  }
  if (!previous?.testMode && previous?.mvpActual) {
    payload.mvpActual = previous.mvpActual;
    payload.mvpActualAt = previous.mvpActualAt || previous.fetchedAt || null;
    payload.mvpActualSource = previous.mvpActualSource || 'manual';
  } else if (finalFinishedIn(stableMatches)) {
    const actual = topScorerId(liveScorers.length ? liveScorers : previous?.mvpLiveScorers);
    if (actual) {
      payload.mvpActual = actual;
      payload.mvpActualAt = payload.fetchedAt;
      payload.mvpActualSource = liveScorers.length ? 'api-scorers' : 'api-scorers-cache';
    }
  }
  ...
  return {
    ...
    mvpLiveScorers: payload.mvpLiveScorers?.length || 0,
  };
}
