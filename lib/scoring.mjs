...
import { buildTree } from './bracket.mjs';
...
function makeBranch(code, type) {
  return { code, team: branchTeam(code), type, state: 'live', value: 3, lastWon: 0 };
}
...
export function evaluateBracket(username) {
  ...
  for (const node of treeNodes(tree)) {
    ...
    const recoveredLive = recoveryPick && (
      branches[recoveryPick]?.state === 'live' || !branches[recoveryPick]
    ) && codeInMatch(recoveryPick, match);
    const recoveryOpen = !!pred.submitted && node.round !== 'R32' && realKnown && beforeKickoff && !originalLive && !recoveryPick;
    const activePick = originalLive ? originalPick : (recoveredLive ? recoveryPick : null);
    const activeType = originalLive ? 'original' : (recoveredLive ? 'recovered' : null);
    const branch = activePick ? ensure(activePick, activeType) : null;
    const value = branch?.value || 3;
    ...
    nodes.push({
      ...
      hit: wonPoints > 0,
      points: wonPoints,
      nextValue: wonPoints > 0 ? value + 1 : null,
      recoveryOpen,
      recoveryOptions: recoveryOpen ? matchTeams(match) : [],
    });
  }
