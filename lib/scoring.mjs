...
const ROUND_LABELS = { R32: '1/16', R16: '8vos', QF: '4tos', SF: 'Semifinales', FINAL: 'Final' };
...
function roundBeforeKickoff(round, tree, matchesByNode) {
  const nodes = nodesByRound(tree)[round] || [];
  const matches = nodes.map(n => matchesByNode[n.id]).filter(m => m?.utcDate);
  return matches.length > 0 && matches.every(m => now() < new Date(m.utcDate));
}
...
function effectiveBracketPicks(pred, picks) {
  const out = { ...(picks || {}) };
  Object.entries(pred?.recoveries || {}).forEach(([nodeId, r]) => {
...
export function evaluateBracket(username) {
  ...
  const projectedCandidates = candidatesFor(tree, resolveR32Teams(), effectiveBracketPicks(pred, picks));
  const roundByNode = Object.fromEntries(treeNodes(tree).map(n => [n.id, n.round]));
  const parentByChild = {};
  ...
  for (const node of treeNodes(tree)) {
    ...
    const beforeRoundKickoff = roundBeforeKickoff(node.round, tree, matchesByNode);
    ...
    const projected = projectedCandidates[node.id] || {};
    const projectedCodes = [projected.a?.code, projected.b?.code].filter(Boolean);
    const realCodes = matchTeams(match).map(t => t.code);
    const projectedMatchChanged = realCodes.length === 2 && !sameCodeSet(projectedCodes, realCodes);
    const projectedOverlap = projectedCodes.filter(code => realCodes.includes(code));
    const recoveryStartRound = recoveryMeta?.startedAt ? roundByNode[recoveryMeta.startedAt] : null;
    const reviewedThisRound = !!(recoveryMeta?.reviewed && recoveryStartRound && roundRank(recoveryStartRound) === roundRank(node.round));
    const canChooseAgain = !!pred.submitted && node.round !== 'R32' && realKnown && beforeKickoff && beforeRoundKickoff && !reviewedThisRound && projectedMatchChanged;
    const canActNow = canChooseAgain && previousRoundClosed;
    const recoveryPending = canChooseAgain && !previousRoundClosed && projectedOverlap.length === 0;
    const recoveryOpen = canActNow && projectedOverlap.length === 0;
    const revisionOpen = canActNow && projectedOverlap.length > 0;
  ...
  const optionalActions = nodes.filter(n => n.revisionOpen).length;
