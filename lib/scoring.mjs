// ============================================================================
//  Puntuación: grupos (exacto + ganador/empate) y llave (avances + bonus)
// ============================================================================
import { CONFIG } from '../config.mjs';
import { db } from './store.mjs';
import { groupMatches, isFinished, actualReached, actualFinal, actualMvp } from './data.mjs';
...
export function userBracketTotals(username) {
  ...
  const finalists = inSet(ur.FINAL, ar.FINAL); // finalistas acertados

  const points = correct * s.perWinner + top4 * s.top4Bonus + finalists * (s.finalistBonus || 0);
  return { points, correct, top4, finalists, hasResult: ar.R16.length > 0 || ar.CHAMP.length > 0 };
}
