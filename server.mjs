...
function recoveryCandidatesFor(tree, r32Teams, picks, detail, forcedIds = []) {
  ...
  const winner = (id) => {
    ...
    if (nd?.resolved && nd.actualWinner) {
      return [c.a, c.b].find(t => t?.code === nd.actualWinner) || nd.actualWinnerTeam || teamFromCode(nd.actualWinner);
    }
...
function buildState(user) {
  ...
  if (bracketDetail.optionalActions) actions.push({ tab: 'llave', level: 'info', badge: true, label: 'Seguimiento opcional', text: `${bracketDetail.optionalActions} cambio${bracketDetail.optionalActions === 1 ? '' : 's'} opcional${bracketDetail.optionalActions === 1 ? '' : 'es'} disponible${bracketDetail.optionalActions === 1 ? '' : 's'} antes de ${madridShortDate(optionalDeadline)}.` });
